...
import {getAllNotes, updateNote} from "@/services/localstorage.ts";
...
function EditNoteModal({isOpen, onClose}: ModalPropsNew) {
    ...
    const selectedItem = useSelector<IRootStore, Note | undefined>((state) => state.ui.selectedNote);

    const onSubmit: SubmitHandler<Inputs> = (data) => {
        console.log('AFAS?')
        updateNote(`${selectedItem?.id}`, `${data.content}`);
        dispatch(uiActions.setNotes(getAllNotes()))
        onClose()
        toast.success(`The note for "${selectedItem?.book_title}" has been updated.`, {
            position: "top-right",
            autoClose: 3000,
    ...
    useEffect(() => {
        if (isOpen && selectedItem) {
            setValue('content', selectedItem?.content || ""); // Set state to selectedItem's content
        }
    }, [isOpen, selectedItem]);
    ...
    const body = (
        <div>
            ...
            <div className="text-lg font-bold text-gray-900 mt-4">{selectedItem?.book_title}</div>
    ...
    return (
        <Modal onSubmit={handleSubmit(onSubmit)} isOpen={isOpen} onClose={onClose} title="Edit Notes" body={body} footer={footer}/>
    )
}
